...
- 可在 UI 中替换本次故事背景，建档后进入每名角色的隔离观察与决策
- 按轮显示四名角色的显式对白，物理动作另以简短系统转述呈现
...
1. 在左侧“故事工作室”替换本次故事背景，再新建运行档案；
2. 点击“快速演示”加载完整轨迹，或“单步新建”逐轮观看角色发言；
3. 在“运行台”查看实时通讯对白，在“角色视角”比较各自真正知道的信息；
4. 在“回放”拖动轮次检查状态变化，在“章节与导出”下载脱敏演示包。
...
演示 UI 限量使用官方角色页公开发布的四张角色立绘与四段短语音，并将其中一张立绘经 AI 重构为通讯舱氛围主视觉；界面和 `app/assets/README.md` 均标注来源与处理方式。项目不再分发角色模型、CG、完整语音包、游戏脚本、Logo、字体或长段台词，也不使用泄露、测试服或未公开材料。
